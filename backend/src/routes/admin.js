...
// ✅ Add Student (profile + login)
router.post("/students", auth, role("admin"), uploadStudent.single("photo"), async (req, res) => {
  try {
    const {
      firstName,
      lastName,
    ...
    if (exists) return res.status(400).json({ message: "Email/UserId already in use" });
    ...
    const newStudent = await Student.create({
      ...
      fees,
      userId,
      password, // will be hashed by Student schema
      photo: req.file ? `/uploads/students/${req.file.filename}` : null,
    });

    // Also create login entry in User collection
    const hashed = await bcrypt.hash(password, 10);
    await User.create({
...
// ✅ Update Student
router.put("/students/:id", auth, role("admin"), uploadStudent.single("photo"), async (req, res) => {
  try {
    const updates = { ...req.body };
    if (updates.password) updates.password = await bcrypt.hash(updates.password, 10);
